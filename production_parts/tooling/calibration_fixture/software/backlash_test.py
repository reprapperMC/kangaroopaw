...
class backlashTest():
    ...
    def run_backlash_test(self, devices, gcode):
        with open(gcode, 'r') as gcode:
            gcode_lines = gcode.read().splitlines()
        print(gcode_lines)
        reading = []
    ...
    def x_axis_backlash_test(self, devices):
        reading = self.run_backlash_test(devices, self.x_dial_calibration)
        return reading

    def y_axis_backlash_test(self, devices):
        reading = self.run_backlash_test(devices, self.y_dial_calibration)
        return reading

    def z_axis_backlash_test(self, devices):
        reading = self.run_backlash_test(devices, self.z_dial_calibration)
        return reading
