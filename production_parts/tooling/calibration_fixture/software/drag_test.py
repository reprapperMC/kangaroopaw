...
class dragTest():
    ...
    def printer_x_level(self, devices):
        with open('gcodes/machine_X_level.gcode', 'r') as gcode:
    ...
    def z_axis_drag_test(self, devices, command):
        zero_gauge = self.nextech.zero_gauge(devices, 'dfs_xz')
        self.printer_z_prep(devices)
        sleep(10.0)
        reading = []
        t = threading.Timer(4, self.z_drag_actuate, args=(devices,)).start()
        reading_count = 80
        while reading_count != 0:
            reading.append(self.nextech.get_clean_force_reading(devices, 'dfs_xz', 'x'))
            reading_count -= 1
