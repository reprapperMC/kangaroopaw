...
class dragTest():
    ...
    def printer_xy_prep(self, devices):
        with open('gcodes/machine_XY_drag.gcode', 'r') as gcode:
            gcode_lines = gcode.read().splitlines()
        for line in gcode_lines:
            devices.ports['printer'].write((line + '\r\n').encode('utf-8'))
    ...
    def x_drag_actuate(self, devices):
        with open('gcodes/fixture_X_drag_calibration.gcode', 'r') as gcode:
            gcode_lines = gcode.read().splitlines()
        for line in gcode_lines:
            devices.ports['actuator'].write((line + '\r\n').encode('utf-8'))
    ...
    def y_axis_drag_test(self, devices, command):
        ...
        sleep(15.0)
        reading = []
        t = threading.Timer(6, self.y_drag_actuate, args=(devices,)).start()
        reading_count = 100
        while reading_count != 0:
            reading.append(self.nextech.get_clean_force_reading(devices, 'dfs_xz', 'x'))
            reading_count -= 1
